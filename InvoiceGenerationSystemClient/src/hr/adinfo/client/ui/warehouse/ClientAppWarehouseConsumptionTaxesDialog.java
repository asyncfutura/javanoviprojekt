 ...
 * @author Matej
 */
public class ClientAppWarehouseConsumptionTaxesDialog extends javax.swing.JDialog {
	private ArrayList<Integer> tableIdList = new ArrayList<>();
	
	/**
	 * Creates new form ClientAppWarehouseCategoriesDialog
	 */
	public ClientAppWarehouseConsumptionTaxesDialog(java.awt.Frame parent, boolean modal) {
		super(parent, modal);
		initComponents();
	...
	private void RefreshTable(){
		ClientAppUtils.CreateConsumptionTaxAmountsIfNoExist(Licence.GetOfficeNumber());
		
		final JDialog loadingDialog = new LoadingDialog(null, true);
		
		String query = "SELECT CONSUMPTION_TAXES.ID, CONSUMPTION_TAXES.NAME, CONSUMPTION_TAX_VALUES.VALUE "
				...
				+ "WHERE CONSUMPTION_TAXES.IS_DELETED = 0 AND CONSUMPTION_TAX_VALUES.OFFICE_NUMBER = ?";
		...
		if(!databaseQueryTask.isDone()){
			...
			try {
				...
				if(databaseQueryResult != null){
					...
					customTableModel.setColumnIdentifiers(new String[] {"Šifra", "Naziv poreza na potrošnju", "Postotak"});
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Porez na potrošnju");
    ...
    private void jButtonEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonEditActionPerformed
        ...
        ClientAppWarehouseConsumptionTaxesAddEditDialog addEditdialog = new ClientAppWarehouseConsumptionTaxesAddEditDialog(null, true, tableId);
        addEditdialog.setVisible(true);
        if(addEditdialog.changeSuccess){
            RefreshTable();
        }
    ...
    private void jButtonDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonDeleteActionPerformed
		...
		int rowId = jTable1.convertRowIndexToModel(jTable1.getSelectedRow());
        int tableId = tableIdList.get(rowId);
		if(tableId < Values.CONSUMPTION_TAXES_MAX_DEFAULT_ID){
            ClientAppLogger.GetInstance().ShowMessage("Nije moguće brisati unaprijed zadane poreze na potrošnju");
            return;
        }
        String tableValue = String.valueOf(jTable1.getModel().getValueAt(rowId, 1));
		
		{
			String queryArticles = "SELECT ID, NAME FROM ARTICLES WHERE IS_DELETED = 0 AND CONSUMPTION_TAX_ID = ?";
				...
				try {
					...
					if(databaseQueryResult != null){
						...
						if (!"".equals(articlesList)){
							ClientAppLogger.GetInstance().ShowMessage("Porez na potrošnju " + tableValue + " se koristi u sljedećim artiklima. Potrebno je promijeniti porez na potrošnju tih artikala kako bi se ona mogla obrisati." + System.lineSeparator() + articlesList);
							isUsed = true;
						}
        ...
        int dialogResult = JOptionPane.showConfirmDialog (null, "Jeste li sigurni da želite obrisati porez na potrošnju " + tableValue, "Obriši porez na potrošnju", JOptionPane.YES_NO_OPTION);
        if(dialogResult == JOptionPane.YES_OPTION){
            final JDialog loadingDialog = new LoadingDialog(null, true);
            

            String query = "UPDATE CONSUMPTION_TAXES SET IS_DELETED = ? WHERE ID = ?";
            DatabaseQuery databaseQuery = new DatabaseQuery(query);
    ...
    private void jButtonPrintPosActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonPrintPosActionPerformed
        PrintUtils.PrintPosTable("Porezi na potrošnju", jTable1);
    ...
    private void jButtonPrintA4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonPrintA4ActionPerformed
        PrintUtils.PrintA4Table("PoreziNaPotrošnju", "Porezi na potrošnju", jTable1, "");
