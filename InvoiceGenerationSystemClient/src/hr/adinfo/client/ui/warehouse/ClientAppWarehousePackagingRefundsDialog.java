 ...
 * @author Matej
 */
public class ClientAppWarehousePackagingRefundsDialog extends javax.swing.JDialog {
	private ArrayList<Integer> tableIdList = new ArrayList<>();
	
	/**
	 * Creates new form ClientAppWarehouseCategoriesDialog
	 */
	public ClientAppWarehousePackagingRefundsDialog(java.awt.Frame parent, boolean modal) {
		super(parent, modal);
		initComponents();
		...
		if(!databaseQueryTask.isDone()){
			...
			try {
				...
				if(databaseQueryResult != null){
					...
					customTableModel.setColumnIdentifiers(new String[] {"Šifra", "Naziv povratne naknade", "Iznos"});
    ...
    private void initComponents() {
        ...
        setTitle("Povratne naknade");
        setResizable(false);

        jLabel1.setFont(new java.awt.Font("Tahoma", 0, 18)); // NOI18N
        jLabel1.setText("Povratne naknade");
    ...
    private void jButtonEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonEditActionPerformed
		...
		if(tableId == 0){
            ClientAppLogger.GetInstance().ShowMessage("Ova unaprijed zadana povratna naknada (" + tableValue + ") ne može se uređivati");
            return;
        }
		
        ClientAppWarehousePackagingRefundsAddEditDialog addEditdialog = new ClientAppWarehousePackagingRefundsAddEditDialog(null, true, tableId);
        addEditdialog.setVisible(true);
        if(addEditdialog.changeSuccess){
            RefreshTable();
        }
    ...
    private void jButtonDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonDeleteActionPerformed
		...
		if(tableId == 0){
            ClientAppLogger.GetInstance().ShowMessage("Ova unaprijed zadana povratna naknada (" + tableValue + ") ne može se brisati");
            return;
        }
		
		{
			String queryTradingGoods = "SELECT ID, NAME FROM TRADING_GOODS WHERE IS_DELETED = 0 AND PACKAGING_REFUND_ID = ?";
				...
				try {
					...
					if(databaseQueryResult != null){
						...
						while (databaseQueryResult[0].next()) {
							tradingGoodsList += (databaseQueryResult[0].getInt(0) + " - " + databaseQueryResult[0].getString(1) + System.lineSeparator());
						}
						
						boolean isUsed = false;
						if (!"".equals(tradingGoodsList)){
							ClientAppLogger.GetInstance().ShowMessage("Povratna naknada " + tableValue + " se koristi u sljedećoj trgovačkoj robi. Potrebno je promijeniti povratnu naknadu te trgovačke robe kako bi se ona mogla obrisati." + System.lineSeparator() + tradingGoodsList);
							isUsed = true;
						}
        ...
        int dialogResult = JOptionPane.showConfirmDialog (null, "Jeste li sigurni da želite obrisati povratnu naknadu " + tableValue, "Obriši povratnu naknadu", JOptionPane.YES_NO_OPTION);
        if(dialogResult == JOptionPane.YES_OPTION){
            final JDialog loadingDialog = new LoadingDialog(null, true);
            

            String query = "UPDATE PACKAGING_REFUNDS SET IS_DELETED = ? WHERE ID = ?";
    ...
    private void jButtonPrintPosActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonPrintPosActionPerformed
		PrintUtils.PrintPosTable("Povratne naknade", jTable1);
    ...
    private void jButtonPrintA4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonPrintA4ActionPerformed
        PrintUtils.PrintA4Table("PovratneNaknade", "Povratne naknade", jTable1, "");
