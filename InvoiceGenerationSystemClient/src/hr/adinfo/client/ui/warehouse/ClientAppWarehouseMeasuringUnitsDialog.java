 ...
 * @author Matej
 */
public class ClientAppWarehouseMeasuringUnitsDialog extends javax.swing.JDialog {
	private ArrayList<Integer> tableIdList = new ArrayList<>();
	
	/**
	 * Creates new form ClientAppWarehouseCategoriesDialog
	 */
	public ClientAppWarehouseMeasuringUnitsDialog(java.awt.Frame parent, boolean modal) {
		super(parent, modal);
		initComponents();
		...
		if(!databaseQueryTask.isDone()){
			...
			try {
				...
				if(databaseQueryResult != null){
					...
					customTableModel.setColumnIdentifiers(new String[] {"Šifra", "Mjerna jedinica"});
    ...
    private void initComponents() {
        ...
        setTitle("Mjerne jedinice");
        setResizable(false);

        jLabel1.setFont(new java.awt.Font("Tahoma", 0, 18)); // NOI18N
        jLabel1.setText("Mjerne jedinice");
    ...
    private void jButtonEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonEditActionPerformed
		...
		if(tableId < Values.MEASURING_UNITS_MAX_DEFAULT_ID){
            ClientAppLogger.GetInstance().ShowMessage("Nije moguće uređivati unaprijed zadane mjerne jedinice");
            return;
        }
		
        ClientAppWarehouseMeasuringUnitsAddEditDialog addEditdialog = new ClientAppWarehouseMeasuringUnitsAddEditDialog(null, true, tableId);
        addEditdialog.setVisible(true);
        if(addEditdialog.changeSuccess){
            RefreshTable();
        }
    ...
    private void jButtonDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonDeleteActionPerformed
        ...
        int tableId = tableIdList.get(rowId);
		if(tableId < Values.MEASURING_UNITS_MAX_DEFAULT_ID){
            ClientAppLogger.GetInstance().ShowMessage("Nije moguće brisati unaprijed zadane mjerne jedinice");
            return;
        }
        String tableValue = String.valueOf(jTable1.getModel().getValueAt(rowId, 1));
		
		{
			String queryMaterials = "SELECT ID, NAME FROM MATERIALS WHERE IS_DELETED = 0 AND MEASURING_UNIT_ID = ?";
			String queryArticles = "SELECT ID, NAME FROM ARTICLES WHERE IS_DELETED = 0 AND MEASURING_UNIT_ID = ?";
			String queryServices = "SELECT ID, NAME FROM SERVICES WHERE IS_DELETED = 0 AND MEASURING_UNIT_ID = ?";
				...
				try {
					...
					if(databaseQueryResult != null){
						...
						if (!"".equals(materialsList)){
							ClientAppLogger.GetInstance().ShowMessage("Mjerna jedinica " + tableValue + " se koristi u sljedećim materijalima. Potrebno je promijeniti mjernu jedincu tih materijala kako bi se ona mogla obrisati." + System.lineSeparator() + materialsList);
							isUsed = true;
						}
						if (!"".equals(articlesList)){
							ClientAppLogger.GetInstance().ShowMessage("Mjerna jedinica " + tableValue + " se koristi u sljedećim artiklima. Potrebno je promijeniti mjernu jedincu tih artikala kako bi se ona mogla obrisati." + System.lineSeparator() + articlesList);
							isUsed = true;
						}
						if (!"".equals(servicesList)){
							ClientAppLogger.GetInstance().ShowMessage("Mjerna jedinica " + tableValue + " se koristi u sljedećim uslugama. Potrebno je promijeniti mjernu jedincu tih usluga kako bi se ona mogla obrisati." + System.lineSeparator() + servicesList);
							isUsed = true;
						}
        ...
        int dialogResult = JOptionPane.showConfirmDialog (null, "Jeste li sigurni da želite obrisati mjernu jedinicu " + tableValue, "Obriši mjernu jedinicu", JOptionPane.YES_NO_OPTION);
        if(dialogResult == JOptionPane.YES_OPTION){
            final JDialog loadingDialog = new LoadingDialog(null, true);
            

            String query = "UPDATE MEASURING_UNITS SET IS_DELETED = ? WHERE ID = ?";
    ...
    private void jButtonPrintA4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonPrintA4ActionPerformed
        PrintUtils.PrintA4Table("MjerneJedinice" , "Mjerne jedinice", jTable1, "");
    ...
    private void jButtonPrintPosActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonPrintPosActionPerformed
		PrintUtils.PrintPosTable("Mjerne jedinice", jTable1);
