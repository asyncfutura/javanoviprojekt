 ...
 * @author Matej
 */
public class ClientAppWarehouseTradingGoodsDialog extends javax.swing.JDialog {
	private ArrayList<Integer> tableIdList = new ArrayList<>();

	/**
	 * Creates new form ClientAppWarehouseMaterialsDialog
	 */
	public ClientAppWarehouseTradingGoodsDialog(java.awt.Frame parent, boolean modal) {
		super(parent, modal);
		initComponents();
	...
	private void RefreshTable(){
		...
		String localInvoiceAmountsSubquery = "SELECT COALESCE(SUM(LOCAL_INVOICE_ITEMS.AMT), 0) "
		...
		String query = "SELECT TRADING_GOODS.ID, TRADING_GOODS.NAME, TRADING_GOODS.PRICE, TAX_RATES.VALUE, "
				+ "TRADING_GOODS_AMOUNTS.AMOUNT - (" + localInvoiceAmountsSubquery + "), "
				+ "TRADING_GOODS.LAST_PRICE, TRADING_GOODS.CUSTOM_ID, TRADING_GOODS.IS_ACTIVE "
		...
		if(!isProduction){
			String invoiceAmountsSubquery = localInvoiceAmountsSubquery.replace(" AND LOCAL_INVOICES.IS_DELETED = 0", "").replace("LOCAL_INVOICE_ITEMS", "INVOICE_ITEMS").replace("LOCAL_INVOICES", "INVOICES");
			String invoiceTestAmountsSubquery = localInvoiceAmountsSubquery.replace(" AND LOCAL_INVOICES.IS_DELETED = 0", "").replace("LOCAL_INVOICE_ITEMS", "INVOICE_ITEMS_TEST").replace("LOCAL_INVOICES", "INVOICES_TEST");
			String localInvoiceTestAmountsSubquery = localInvoiceAmountsSubquery.replace("LOCAL_INVOICE_ITEMS", "LOCAL_INVOICE_ITEMS_TEST").replace("LOCAL_INVOICES", "LOCAL_INVOICES_TEST");
			query = "SELECT TRADING_GOODS.ID, TRADING_GOODS.NAME, TRADING_GOODS.PRICE, TAX_RATES.VALUE, "
				+ "TRADING_GOODS_AMOUNTS.AMOUNT + (" + invoiceAmountsSubquery + ") - (" + invoiceTestAmountsSubquery + ") - (" + localInvoiceTestAmountsSubquery + "), "
				+ "TRADING_GOODS.LAST_PRICE, TRADING_GOODS.CUSTOM_ID, TRADING_GOODS.IS_ACTIVE "
				+ "FROM ((TRADING_GOODS INNER JOIN TAX_RATES ON TRADING_GOODS.TAX_RATE_ID = TAX_RATES.ID) "
				+ "INNER JOIN TRADING_GOODS_AMOUNTS ON TRADING_GOODS.ID = TRADING_GOODS_AMOUNTS.TRADING_GOODS_ID) "
				+ "WHERE TRADING_GOODS.IS_DELETED = 0 AND TRADING_GOODS_AMOUNTS.OFFICE_NUMBER = ? "
		...
		if(!databaseQueryTask.isDone()){
			...
			try {
				...
				if(databaseQueryResult != null){
					...
					customTableModel.setColumnIdentifiers(new String[] {"Šifra", "Naziv", "Porezna stopa", "Cijena", "PNC", "Na skladištu"});
					...
					while (databaseQueryResult.next()) {
						...
						rowData[4] = ClientAppUtils.FloatToPriceString(databaseQueryResult.getFloat(5));
						rowData[5] = databaseQueryResult.getString(4);
					...
					jTable1.getColumnModel().getColumn(2).setPreferredWidth(jScrollPane1.getWidth() * 16 / 100);
					jTable1.getColumnModel().getColumn(3).setPreferredWidth(jScrollPane1.getWidth() * 13 / 100);
					jTable1.getColumnModel().getColumn(4).setPreferredWidth(jScrollPane1.getWidth() * 13 / 100);
					jTable1.getColumnModel().getColumn(5).setPreferredWidth(jScrollPane1.getWidth() * 13 / 100);
    ...
    private void initComponents() {
        ...
        jLabel9.setText("Trgovačka roba");
        ...
        jTextFieldSearch.setPreferredSize(new java.awt.Dimension(200, 25));
    ...
    private void jButtonAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonAddActionPerformed
		ClientAppWarehouseTradingGoodsAddEditDialog addEditdialog = new ClientAppWarehouseTradingGoodsAddEditDialog(null, true, -1);
    ...
    private void jButtonEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonEditActionPerformed
        ...
        ClientAppWarehouseTradingGoodsAddEditDialog addEditdialog = new ClientAppWarehouseTradingGoodsAddEditDialog(null, true, tableId);
        addEditdialog.setVisible(true);
        if(addEditdialog.changeSuccess){
            RefreshTable();
        }
    ...
    private void jButtonDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonDeleteActionPerformed
        ...
        if(dialogResult == JOptionPane.YES_OPTION){
            ...
            String query = "UPDATE TRADING_GOODS SET IS_DELETED = ? WHERE ID = ?";
            DatabaseQuery databaseQuery = new DatabaseQuery(query);
    ...
    private void jButtonPrintA4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonPrintA4ActionPerformed
		PrintUtils.PrintA4Table("TrgovačkaRoba", "Trgovačka roba", jTable1, "");
    ...
    private void jButtonPrintPosActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonPrintPosActionPerformed
		PrintUtils.PrintPosTable("Trgovačka roba", jTable1, new int[]{0, 1, 3, 5});
    ...
    private void jButtonPrintOnDateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonPrintOnDateActionPerformed
		...
		String queryTradingGoods = "SELECT TRADING_GOODS.ID, TRADING_GOODS.NAME, TRADING_GOODS.LAST_PRICE "
				+ "FROM TRADING_GOODS "
				+ "WHERE TRADING_GOODS.IS_DELETED = 0 "
				+ "ORDER BY TRADING_GOODS.ID";
		multiDatabaseQuery.SetQuery(0, queryTradingGoods);
		
		String queryReceipts = "SELECT RECEIPT_TRADING_GOODS.TRADING_GOODS_ID, SUM(RECEIPT_TRADING_GOODS.AMOUNT) "
				+ "FROM RECEIPTS "
				...
				+ "AND RECEIPTS.IS_DELETED = 0 AND RECEIPT_TRADING_GOODS.IS_DELETED = 0 "
				+ "GROUP BY RECEIPT_TRADING_GOODS.TRADING_GOODS_ID";
		...
		multiDatabaseQuery.AddParam(2, 3, Values.SETTINGS_LAYOUT_ITEM_TYPE_TRADINGGOODS);
		multiDatabaseQuery.AddParam(2, 4, Licence.GetOfficeNumber());
		
		String queryLocalInvoices = "SELECT LOCAL_INVOICE_ITEMS.IT_ID, SUM(LOCAL_INVOICE_ITEMS.AMT) "
				+ "FROM LOCAL_INVOICE_ITEMS "
				+ "INNER JOIN LOCAL_INVOICES ON LOCAL_INVOICE_ITEMS.IN_ID = LOCAL_INVOICES.ID "
				+ "WHERE LOCAL_INVOICES.I_DATE >= ? AND LOCAL_INVOICES.I_DATE <= ? "
				+ "AND LOCAL_INVOICE_ITEMS.IT_TYPE = ? "
				+ "AND LOCAL_INVOICES.O_NUM = ? "
				+ "AND LOCAL_INVOICES.PAY_TYPE <> " + Values.PAYMENT_METHOD_TYPE_OFFER + " "
				+ "AND LOCAL_INVOICES.PAY_TYPE <> " + Values.PAYMENT_METHOD_TYPE_SUBTOTAL + " "
				+ "GROUP BY LOCAL_INVOICE_ITEMS.IT_ID";
		if(!isProduction){
		...
		multiDatabaseQuery.AddParam(3, 3, Values.SETTINGS_LAYOUT_ITEM_TYPE_TRADINGGOODS);
			...
			try {
				...
				if(databaseQueryResult != null){
					...
					while (databaseQueryResult[index].next()) {
						...
						amountList.add(0f);
						lastPriceList.add(databaseQueryResult[index].getFloat(2));
					}
		...
		PrintUtils.PrintA4Table("TragovačkaRoba-StanjeKrozPeriod", "Stanje skladišta kroz period - Trgovačka roba", tempJTable, new int[]{0, 1, 2, 3, 4, 5}, new int[]{}, printTableExtraData, "");
		tempJTable = null;
