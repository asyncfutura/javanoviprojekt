 ...
 * @author Matej
 */
public class ClientAppWarehouseTaxRatesDialog extends javax.swing.JDialog {
	/**
	...
	public ClientAppWarehouseTaxRatesDialog(java.awt.Frame parent, boolean modal) {
		super(parent, modal);
		initComponents();
		...
		if(!databaseQueryTask.isDone()){
			...
			try {
				...
				if(databaseQueryResult != null){
					...
					customTableModel.setColumnIdentifiers(new String[] {"Šifra", "Naziv porezne stope", "Postotak"});
    ...
    private void initComponents() {
        ...
        setTitle("Porezne stope");
        setResizable(false);

        jLabel1.setFont(new java.awt.Font("Tahoma", 0, 18)); // NOI18N
        jLabel1.setText("Porezne stope");
    ...
    private void jButtonEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonEditActionPerformed
		...
		if(tableId == 0 || tableId == 2){
            ClientAppLogger.GetInstance().ShowMessage("Ova unaprijed zadana porezna stopa (" + tableValue + ") ne može se uređivati");
            return;
        }
		
        ClientAppWarehouseTaxRatesAddEditDialog addEditdialog = new ClientAppWarehouseTaxRatesAddEditDialog(null, true, tableId);
        addEditdialog.setVisible(true);
        if(addEditdialog.changeSuccess){
            RefreshTable();
        }
    ...
    private void jButtonDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonDeleteActionPerformed
		...
		if(tableId == 0 || tableId == 1 || tableId == 2){
            ClientAppLogger.GetInstance().ShowMessage("Ova unaprijed zadana porezna stopa (" + tableValue + ") ne može se brisati");
			return;
        }
		
		{
			String queryArticles = "SELECT ID, NAME FROM ARTICLES WHERE IS_DELETED = 0 AND TAX_RATE_ID = ?";
			String queryTradingGoods = "SELECT ID, NAME FROM TRADING_GOODS WHERE IS_DELETED = 0 AND TAX_RATE_ID = ?";
			String queryServices = "SELECT ID, NAME FROM SERVICES WHERE IS_DELETED = 0 AND TAX_RATE_ID = ?";
			...
			multiDatabaseQuery.SetQuery(2, queryServices);
				...
				try {
					...
					if(databaseQueryResult != null){
						...
						while (databaseQueryResult[1].next()) {
							tradingGoodsList += (databaseQueryResult[1].getInt(0) + " - " + databaseQueryResult[1].getString(1) + System.lineSeparator());
						}
						while (databaseQueryResult[2].next()) {
							servicesList += (databaseQueryResult[2].getInt(0) + " - " + databaseQueryResult[2].getString(1) + System.lineSeparator());
						}
						
						boolean isUsed = false;
						if (!"".equals(articlesList)){
							ClientAppLogger.GetInstance().ShowMessage("Porezna stopa " + tableValue + " se koristi u sljedećim artiklima. Potrebno je promijeniti kategoriju tih artikala kako bi se ona mogla obrisati." + System.lineSeparator() + articlesList);
							isUsed = true;
						}
						if (!"".equals(tradingGoodsList)){
							ClientAppLogger.GetInstance().ShowMessage("Porezna stopa " + tableValue + " se koristi u sljedećoj trgovačkoj robi. Potrebno je promijeniti kategoriju te trgovačke robe kako bi se ona mogla obrisati." + System.lineSeparator() + tradingGoodsList);
							isUsed = true;
						}
						if (!"".equals(servicesList)){
							ClientAppLogger.GetInstance().ShowMessage("Porezna stopa " + tableValue + " se koristi u sljedećim uslugama. Potrebno je promijeniti kategoriju tih usluga kako bi se ona mogla obrisati." + System.lineSeparator() + servicesList);
							isUsed = true;
						}
        ...
        int dialogResult = JOptionPane.showConfirmDialog (null, "Jeste li sigurni da želite obrisati poreznu stopu " + tableValue, "Obriši poreznu stopu", JOptionPane.YES_NO_OPTION);
        if(dialogResult == JOptionPane.YES_OPTION){
            final JDialog loadingDialog = new LoadingDialog(null, true);
            

            String query = "UPDATE TAX_RATES SET IS_DELETED = ? WHERE ID = ?";
    ...
    private void jButtonPrintPosActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonPrintPosActionPerformed
        PrintUtils.PrintPosTable("Porezne stope", jTable1);
    ...
    private void jButtonPrintA4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonPrintA4ActionPerformed
		PrintUtils.PrintA4Table("PorezneStope", "Porezne stope", jTable1, "");
