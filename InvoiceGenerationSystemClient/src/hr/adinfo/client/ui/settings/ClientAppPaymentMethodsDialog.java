 ...
 * @author Matej
 */
public class ClientAppPaymentMethodsDialog extends javax.swing.JDialog {
	private ArrayList<Integer> tableIdList = new ArrayList<>();
	
	/**
	 * Creates new form ClientAppWarehouseCategoriesDialog
	 */
	public ClientAppPaymentMethodsDialog(java.awt.Frame parent, boolean modal) {
		super(parent, modal);
		initComponents();
	...
	private void RefreshTable(){
		...
		DatabaseQuery databaseQuery = new DatabaseQuery("SELECT ID, NAME, PAYMENT_TYPE, IS_ACTIVE FROM PAYMENT_METHODS WHERE IS_DELETED = 0");
		...
		if(!databaseQueryTask.isDone()){
			...
			try {
				...
				if(databaseQueryResult != null){
					...
					customTableModel.setColumnIdentifiers(new String[] {"Način plaćanja", "Fiskalni tip plaćanja", "Aktivan"});
					ArrayList<Integer> idList = new ArrayList<>();
					while (databaseQueryResult.next()) {
						Object[] rowData = new Object[3];
						rowData[0] = databaseQueryResult.getString(1);
						rowData[1] = Values.PAYMENT_METHOD_TYPE_NAMES[databaseQueryResult.getInt(2)];
    ...
    private void initComponents() {
        ...
        setTitle("Načini plaćanja");
        setResizable(false);

        jLabel1.setFont(new java.awt.Font("Tahoma", 0, 18)); // NOI18N
        jLabel1.setText("Načini plaćanja");
    ...
    private void jButtonAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonAddActionPerformed
        ClientAppPaymentMethodsAddEditDialog addEditdialog = new ClientAppPaymentMethodsAddEditDialog(null, true, -1);
    ...
    private void jButtonEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonEditActionPerformed
		...
		if(tableId == 0){
            ClientAppLogger.GetInstance().ShowMessage("Nije moguće uređivati ovaj način plaćanja");
            return;
        }
		
        ClientAppPaymentMethodsAddEditDialog addEditdialog = new ClientAppPaymentMethodsAddEditDialog(null, true, tableId);
        addEditdialog.setVisible(true);
        if(addEditdialog.changeSuccess){
            RefreshTable();
        }
    ...
    private void jButtonDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonDeleteActionPerformed
		...
		if(tableId < Values.PAYMENT_METHODS_MAX_DEFAULT_ID){
            ClientAppLogger.GetInstance().ShowMessage("Nije moguće brisati unaprijed zadane načine plaćanja");
            return;
        }
        String tableValue = String.valueOf(jTable1.getModel().getValueAt(rowId, 1));
		
        int dialogResult = JOptionPane.showConfirmDialog (null, "Jeste li sigurni da želite obrisati način plaćanja " + tableValue, "Obriši način plaćanja", JOptionPane.YES_NO_OPTION);
        if(dialogResult == JOptionPane.YES_OPTION){
            final JDialog loadingDialog = new LoadingDialog(null, true);
            

            String query = "UPDATE PAYMENT_METHODS SET IS_DELETED = ? WHERE ID = ?";
    ...
    private void jButtonPrintA4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonPrintA4ActionPerformed
        PrintUtils.PrintA4Table("NačiniPlaćanja", "Načini plaćanja", jTable1, "");
    ...
    private void jButtonPrintPosActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonPrintPosActionPerformed
        PrintUtils.PrintPosTable("Načini plaćanja", jTable1, new int[]{0, 1, 2});
