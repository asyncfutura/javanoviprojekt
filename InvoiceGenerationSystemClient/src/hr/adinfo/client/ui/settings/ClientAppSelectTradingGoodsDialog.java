...
import hr.adinfo.client.ui.warehouse.ClientAppWarehouseTradingGoodsAddEditDialog;
 ...
 * @author Matej
 */
public class ClientAppSelectTradingGoodsDialog extends javax.swing.JDialog {
	 ...
	 */
	public ClientAppSelectTradingGoodsDialog(java.awt.Frame parent, boolean modal, int currentYear) {
		super(parent, modal);
		initComponents();
	...
	private void RefreshTable(){
		...
		String query = "SELECT TRADING_GOODS.ID, TRADING_GOODS.NAME, TRADING_GOODS.PRICE, "
				+ "TRADING_GOODS_AMOUNTS.AMOUNT - (" + localInvoiceAmountsSubquery + "), TRADING_GOODS.CUSTOM_ID "
				+ "FROM TRADING_GOODS "
				+ "INNER JOIN TRADING_GOODS_AMOUNTS ON TRADING_GOODS.ID = TRADING_GOODS_AMOUNTS.TRADING_GOODS_ID "
				+ "WHERE TRADING_GOODS.IS_DELETED = 0 AND TRADING_GOODS_AMOUNTS.AMOUNT_YEAR = ?";
		...
		if(!isProduction){
			...
			query = "SELECT TRADING_GOODS.ID, TRADING_GOODS.NAME, TRADING_GOODS.PRICE, "
		...
		databaseQuery.AddParam(1, currentYear);
		...
		if(!databaseQueryTask.isDone()){
			...
			try {
				...
				if(databaseQueryResult != null){
					...
					customTableModel.setColumnIdentifiers(new String[] {"Šifra", "Naziv", "Cijena", "Na skladištu"});
					ArrayList<Integer> idList = new ArrayList<>();
					while (databaseQueryResult.next()) {
						Object[] rowData = new Object[4];
						rowData[0] = customIdEnabled ? databaseQueryResult.getString(4) : databaseQueryResult.getString(0);
						rowData[1] = databaseQueryResult.getString(1);
						rowData[2] = ClientAppUtils.FloatToPriceString(databaseQueryResult.getFloat(2));
						rowData[3] = databaseQueryResult.getFloat(3);
    ...
    private void initComponents() {
        ...
        setTitle("Odabir trgovačke robe");
        setResizable(false);
        ...
        jButtonAdd.setText("<html> <div style=\"text-align: center\"> Dodaj novu <br> trg. robu <br> [INS] </div> </html>");
    ...
    private void jButtonSelectActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonSelectActionPerformed
		...
		selectedName = String.valueOf(jTable1.getModel().getValueAt(rowId, 1));
