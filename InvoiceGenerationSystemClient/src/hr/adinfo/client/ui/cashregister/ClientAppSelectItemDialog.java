 ...
 * @author Matej
 */
public class ClientAppSelectItemDialog extends javax.swing.JDialog {
	 ...
	 */
	public ClientAppSelectItemDialog(java.awt.Frame parent, boolean modal, String startFilter) {
		super(parent, modal);
		initComponents();
		...
		try {
			...
			sorter.setRowFilter(RowFilter.regexFilter("(?iu)" + startFilter));
			jTable1.setRowSorter(sorter);
			jTextField1.setText(startFilter);
		} catch (Exception e){}
	...
	private void RefreshTable(){
		...
		String queryArticles = "SELECT ARTICLES.ID, ARTICLES.NAME, ARTICLES.PRICE, MEASURING_UNITS.NAME, ARTICLES.CUSTOM_ID "
				+ "FROM ARTICLES "
				+ "INNER JOIN MEASURING_UNITS ON ARTICLES.MEASURING_UNIT_ID = MEASURING_UNITS.ID "
				+ "WHERE ARTICLES.IS_DELETED = 0";
		String queryServices = "SELECT SERVICES.ID, SERVICES.NAME, SERVICES.PRICE, MEASURING_UNITS.NAME, SERVICES.CUSTOM_ID "
				+ "FROM SERVICES "
				+ "INNER JOIN MEASURING_UNITS ON SERVICES.MEASURING_UNIT_ID = MEASURING_UNITS.ID "
				+ "WHERE SERVICES.IS_DELETED = 0";
		String queryTradingGoods = "SELECT TRADING_GOODS.ID, TRADING_GOODS.NAME, TRADING_GOODS.PRICE, TRADING_GOODS.CUSTOM_ID "
				+ "FROM TRADING_GOODS "
		...
		multiDatabaseQuery.SetQuery(1, queryServices);
		...
		if(!databaseQueryTask.isDone()){
			...
			try {
				...
				if(databaseQueryResults != null){
					...
					customTableModel.setColumnIdentifiers(new String[] {"Šifra", "Naziv", "Cijena", "Mj. jed."});
					...
					jTable1.getColumnModel().getColumn(3).setPreferredWidth(jScrollPane1.getWidth() * 20 / 100);
				}
    ...
    private void initComponents() {
        ...
        jButtonAdd.setText("<html> <div style=\"text-align: center\"> Dodaj novu <br> stavku <br> [INS] </div> </html>");
