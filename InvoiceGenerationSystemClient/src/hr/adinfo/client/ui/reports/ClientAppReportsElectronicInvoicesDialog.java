...
import hr.adinfo.client.ui.cashregister.ClientAppSelectInvoiceDialog;
 ...
 * @author Matej
 */
public class ClientAppReportsElectronicInvoicesDialog extends javax.swing.JDialog {
	 ...
	 */
	public ClientAppReportsElectronicInvoicesDialog(java.awt.Frame parent, boolean modal) {
		super(parent, modal);
		initComponents();
			...
			public boolean dispatchKeyEvent(KeyEvent ke) {
				...
				if(ke.getID() == KeyEvent.KEY_PRESSED){
					if(ke.getKeyCode() == KeyEvent.VK_ESCAPE){
						...
						jButtonNewEinvoice.doClick();
					} else if(ke.getKeyCode() == KeyEvent.VK_F10){
						jButtonUpdateStatus.doClick();
					}
				}
	...
	private void RefreshTable(){
		...
		String queryLocal = "SELECT LOCAL_INVOICES.ID, CR_NUM, I_NUM, I_DATE, I_TIME, S_ID, PAY_NAME, PAY_TYPE, C_ID, LOCAL_INVOICES.DIS_PCT, "
				+ "LOCAL_INVOICES.DIS_AMT, FIN_PR, NOTE, STAFF.FIRST_NAME, STAFF.LAST_NAME, JIR, MIN(LOCAL_INVOICE_ITEMS.AMT), SPEC_NUM, O_TAG, "
				+ "E_IN_ID, E_IN_ST, S_ZKI, S_JIR "
				+ "FROM LOCAL_INVOICES "
				...
				+ "WHERE O_NUM = ? AND LOCAL_INVOICES.IS_DELETED = 0 AND E_IN_ID != '' "
				+ "AND I_DATE >= ? AND I_DATE <= ? AND PAY_TYPE IN (" + payTypesString + ") "
				+ "GROUP BY LOCAL_INVOICES.ID, CR_NUM, I_NUM, SPEC_NUM, O_TAG, E_IN_ID, E_IN_ST, S_ZKI, S_JIR, I_DATE, I_TIME, S_ID, PAY_NAME, PAY_TYPE, C_ID, LOCAL_INVOICES.DIS_PCT, "
		...
		String query = "SELECT INVOICES.ID, CR_NUM, I_NUM, I_DATE, I_TIME, S_ID, PAY_NAME, PAY_TYPE, C_ID, INVOICES.DIS_PCT, "
				+ "INVOICES.DIS_AMT, FIN_PR, NOTE, STAFF.FIRST_NAME, STAFF.LAST_NAME, JIR, MIN(INVOICE_ITEMS.AMT), SPEC_NUM, O_TAG, "
				+ "E_IN_ID, E_IN_ST, S_ZKI, S_JIR "
				+ "FROM INVOICES "
				+ "INNER JOIN STAFF ON STAFF.ID = INVOICES.S_ID "
				+ "INNER JOIN INVOICE_ITEMS ON INVOICES.ID = INVOICE_ITEMS.IN_ID "
				+ "WHERE O_NUM = ? AND E_IN_ID != '' "
				+ "AND I_DATE >= ? AND I_DATE <= ? AND PAY_TYPE IN (" + payTypesString + ") "
				+ "GROUP BY INVOICES.ID, CR_NUM, I_NUM, SPEC_NUM, O_TAG, E_IN_ID, E_IN_ST, S_ZKI, S_JIR, I_DATE, I_TIME, S_ID, PAY_NAME, PAY_TYPE, C_ID, INVOICES.DIS_PCT, "
		...
		if(!databaseQueryTask.isDone()){
			...
			try {
				...
				if(databaseQueryResult != null){
					CustomTableModel customTableModel = new CustomTableModel();
					customTableModel.setColumnIdentifiers(new String[] {"Datum", "Br. rač.", "Br. e-rač.", "Djelatnik", "Nač. plać.", "Tip rač.", "Ukupno", "Status"});
					...
					while (databaseQueryResult[0].next()) {
						...
						rowData[1] = databaseQueryResult[0].getInt(2) + "/" + databaseQueryResult[0].getString(18) + "/" + databaseQueryResult[0].getString(1);
						rowData[2] = databaseQueryResult[0].getString(19);
						...
						rowData[5] = databaseQueryResult[0].getInt(8) == -1 ? "Običan" : "R1";
						...
						rowData[7] = databaseQueryResult[0].getString(20);
					...
					while (databaseQueryResult[1].next()) {
						...
						rowData[1] = databaseQueryResult[1].getInt(2) + "/" + databaseQueryResult[1].getString(18) + "/" + databaseQueryResult[1].getString(1);
						rowData[2] = databaseQueryResult[1].getString(19);
						...
						rowData[5] = databaseQueryResult[1].getInt(8) == -1 ? "Običan" : "R1";
						...
						rowData[6] = ClientAppUtils.FloatToPriceString(totalPrice);
						rowData[7] = databaseQueryResult[1].getString(20);
						customTableModel.addRow(rowData);
					...
					jTableInvoices.getColumnModel().getColumn(1).setPreferredWidth(jScrollPaneInvoices.getWidth() * 13 / 100);
					jTableInvoices.getColumnModel().getColumn(2).setPreferredWidth(jScrollPaneInvoices.getWidth() * 14 / 100);
					jTableInvoices.getColumnModel().getColumn(3).setPreferredWidth(jScrollPaneInvoices.getWidth() * 15 / 100);
					jTableInvoices.getColumnModel().getColumn(4).setPreferredWidth(jScrollPaneInvoices.getWidth() * 14 / 100);
					jTableInvoices.getColumnModel().getColumn(5).setPreferredWidth(jScrollPaneInvoices.getWidth() * 10 / 100);
					jTableInvoices.getColumnModel().getColumn(6).setPreferredWidth(jScrollPaneInvoices.getWidth() * 13 / 100);
					jTableInvoices.getColumnModel().getColumn(7).setPreferredWidth(jScrollPaneInvoices.getWidth() * 30 / 100);
				}
	...
	private void RefreshTableItems(int itemId, boolean isInvoiceLocal, String note){
		...
		if(!databaseQueryTask.isDone()){
			...
			try {
				...
				if(databaseQueryResult != null){
					CustomTableModel customTableModel = new CustomTableModel();
					customTableModel.setColumnIdentifiers(new String[] {"Šifra", "Naziv", "Kol.", "Cijena", "Popust", "Ukupno"});
    ...
    private void initComponents() {
        ...
        jButtonNewEinvoice = new javax.swing.JButton();
        ...
        jButtonPrintPosInvoicesTable.setPreferredSize(new java.awt.Dimension(70, 60));
        ...
        jButtonPrintA4InvoicesTable.setPreferredSize(new java.awt.Dimension(70, 60));
        ...
        jButtonExit.setPreferredSize(new java.awt.Dimension(65, 60));
        ...
        jButtonPrintPosInvoice.setPreferredSize(new java.awt.Dimension(70, 60));
        ...
        jButtonPrintA4Invoice.setPreferredSize(new java.awt.Dimension(70, 60));
        ...
        jButtonPrintPosAllInvoices.setPreferredSize(new java.awt.Dimension(70, 60));
        ...
        jButtonPrintA4AllInvoices.setPreferredSize(new java.awt.Dimension(70, 60));
        ...
        jButtonNewEinvoice.setText("<html> <div style=\"text-align: center\"> Novi <br> e-račun <br> [INS] </div> </html>");
        jButtonNewEinvoice.setMargin(new java.awt.Insets(2, 2, 2, 2));
        jButtonNewEinvoice.setPreferredSize(new java.awt.Dimension(70, 60));
        jButtonNewEinvoice.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButtonNewEinvoiceActionPerformed(evt);
            }
        });

        jButtonUpdateStatus.setText("<html> <div style=\"text-align: center\"> Ažuriraj <br> status <br> [F10] </div> </html>");
        jButtonUpdateStatus.setMargin(new java.awt.Insets(2, 2, 2, 2));
        jButtonUpdateStatus.setPreferredSize(new java.awt.Dimension(70, 60));
        ...
        jPanelButtonsLayout.setHorizontalGroup(
            ...
            .addGroup(jPanelButtonsLayout.createSequentialGroup()
                ...
                .addGap(18, 18, 18)
                .addComponent(jButtonNewEinvoice, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(jButtonUpdateStatus, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addGap(18, 18, 18)
                ...
                .addGroup(jPanelButtonsLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(jPanelButtonsLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                        ...
                        .addGroup(jPanelButtonsLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                            ...
                            .addComponent(jButtonNewEinvoice, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(jButtonUpdateStatus, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE))
    ...
    private void jButtonPrintA4InvoiceActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonPrintA4InvoiceActionPerformed
		...
		// Extra data
		PrintTableExtraData printTableExtraData1 = ClientAppUtils.GetInvoiceExtraDataItems(invoice, "");
    ...
    private void jButtonNewEinvoiceActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonNewEinvoiceActionPerformed
		ClientAppSelectInvoiceDialog dialogInvoice = new ClientAppSelectInvoiceDialog(null, true, "Odaberite račun");
		dialogInvoice.setVisible(true);
		String invoiceAttachment = "";
		if(dialogInvoice.selectedInvoice != null){
			int dialogResult = JOptionPane.showConfirmDialog (null, "Želite li dodati sliku kao prilog e-računu?", "Prilog", JOptionPane.YES_NO_OPTION);
			if (dialogResult == JOptionPane.YES_OPTION){
				JFileChooser chooser = new JFileChooser();
				chooser.setDialogTitle("Učitaj sliku kao prilog");
				...
				if(certFileBytes != null){
					invoiceAttachment = Base64.getEncoder().encodeToString(certFileBytes);	
				}
			}
			
			ElectronicInvoice.UploadInvoice(dialogInvoice.selectedInvoice, invoiceAttachment);
			if("".equals(dialogInvoice.selectedInvoice.einvoiceId)){
				ClientAppLogger.GetInstance().ShowMessage("Pogreška kod slanja e-računa. Molimo pokušajte ponovno.");
			} else {
				ClientAppLogger.GetInstance().ShowMessage("E-račun uspješno predan. Broj e-računa: " + dialogInvoice.selectedInvoice.einvoiceId);
				RefreshTable();
			}
		}
    }//GEN-LAST:event_jButtonNewEinvoiceActionPerformed
    ...
    private void jButtonUpdateStatusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonUpdateStatusActionPerformed
		if(jTableInvoices.getSelectedRow() == -1){
			ClientAppLogger.GetInstance().ShowMessage("Odaberite račun u tablici čiji status želite ažurirati.");
			return;
		}
		...
		Invoice invoice = ClientAppUtils.GetInvoice(tableId, isInvoiceLocal);
		String year = new SimpleDateFormat("YYYY").format(invoice.date);
		ElectronicInvoice.UpdateInvoiceStatus(invoice.officeNumber, invoice.officeTag, invoice.cashRegisterNumber, invoice.invoiceNumber, invoice.specialNumber, year, invoice.einvoiceStatus, invoice.isTest);
		RefreshTable();
    }//GEN-LAST:event_jButtonUpdateStatusActionPerformed

    // Variables declaration - do not modify//GEN-BEGIN:variables
    private javax.swing.JButton jButtonExit;
    private javax.swing.JButton jButtonNewEinvoice;
