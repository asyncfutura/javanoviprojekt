 ...
 * @author Matej
 */
public class ClientAppServerAppClient implements ExecuteQueryInterface {
	private static ClientAppServerAppClient clientAppServerAppClient = null;
	...
	private ClientAppServerAppClient() {
		ConnectSocket();
		ReadResponses();
	}
	...
	private void OnSocketConnected(){
		new Thread(new Runnable() {
			@Override
			public void run() {
				ClientAppLogger.GetInstance().LogMessage("ClientAppServerAppClient connected");
					...
					try {
						...
						if(!ClientApp.appClosing){
							if(ex.getMessage() != null && ex.getMessage().contains("recv failed")){
								ClientAppLogger.GetInstance().LogMessage("ClientAppServerAppClient disconnected (1)");
							} else if(ex.getMessage() != null && ex.getMessage().contains("Connection reset")){
								ClientAppLogger.GetInstance().LogMessage("ClientAppServerAppClient disconnected (2)");
							} else {
								ClientAppLogger.GetInstance().LogError(ex);
							}
	...
	public static void Init(){
		if(clientAppServerAppClient == null){
			clientAppServerAppClient = new ClientAppServerAppClient();
		}
	}
	
	public static ClientAppServerAppClient GetInstance(){
		return clientAppServerAppClient;
	}
