 ...
 * @author Matej
 */
public class ControlAppUpdater {
	private static final int UPDATE_CHECK_DELAY_SECONDS = 60 * 10;
	private static final int APP_VERSION = 61;
    private static final String user = "adinfo";
    private static final String pass = "ad1nf01oo%";
    private static final String urlAppVersion = "http://www.app.london.com.hr/controlAppVersion.txt";
    private static final String urlPrefix = "http://www.app.london.com.hr/";
	private static final String updaterCommand = "java -jar InvoiceGenerationSystemUpdater.jar control";
		...
		try {
			...
			if (scanner.hasNext()) {
				...
				if(newAppVersion > APP_VERSION){
					...
					DownloadUpdate(filesToDownload, urlPrefix);
				}
			}
		} catch (IOException ex) {
			ControlAppLogger.GetInstance().LogError(ex);
		}
	}
	
	private static void DownloadUpdate(ArrayList<String> filesToDownload, String urlPrefix){
		ControlAppLogger.GetInstance().ShowMessage("Update found! Updating..." + System.lineSeparator());
		try {
			new File("temp").mkdir();
			...
			if(ControlApp.GetInstance() != null){
				ControlApp.GetInstance().OnAppClose();
			} else {
				System.exit(0);
			}
		} catch (IOException ex) {
